...
void NcGridCell::InitialiseVectors( const unsigned int lengthTimeArray ) {
    ...
    mWaterCapacity.resize(lengthTimeArray, Constants::cMissingValue);
...
double NcGridCell::GetWaterCapacity(const unsigned int timeIndex) const {
    return mWaterCapacity[ timeIndex ];
}
...
void NcGridCell::SetWaterCapacity(const double WaterCapacity, const unsigned int timeIndex){
    mWaterCapacity[ timeIndex ] = WaterCapacity;
}
