...
Reproduction::Reproduction( std::string globalModelTimeStepUnit, bool drawRandomly ) {
...
void Reproduction::InitializeEcologicalProcess( GridCell& gcl, MadingleyInitialisation& params, std::string implementationKey ) {
    
}

void Reproduction::RunEcologicalProcess( GridCell& gcl, Cohort& actingCohort, unsigned currentTimestep, ThreadLockedParallelVariables& partial, unsigned currentMonth, MadingleyInitialisation& params ) {
    ...
    mImplementations["reproduction basic"]->RunReproductiveMassAssignment( gcl, actingCohort, currentTimestep, params );
    ...
    mImplementations["reproduction basic"]->RunReproductionEvents( gcl, actingCohort, currentTimestep, partial, _Iteroparous, currentMonth, params );
}
