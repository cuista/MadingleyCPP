...
void MortalitySet::RunEcologicalProcess( GridCell& gcl, Cohort& actingCohort, unsigned currentTimestep, ThreadVariables& partial, unsigned currentMonth, MadingleyInitialisation& params ) {
    ...
    for( auto Biomass: Cohort::mIndividualMassAccounting[ "reproductivebiomass" ] ) {
        // Add the delta biomass to net biomass
    ...
    Cohort::mIndividualMassAccounting[ "abundance" ][ "mortality" ] = MortalityTotal;
    ...
    Cohort::mIndividualMassAccounting[ "organicpool" ][ "mortality" ] = ( 1 - MortalityTotal ) * actingCohort.mCohortAbundance * ( BodyMassIncludingChangeThisTimeStep + ReproductiveMassIncludingChangeThisTimeStep );
}
