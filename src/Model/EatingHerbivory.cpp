...
void EatingHerbivory::GetEatingPotentialTerrestrial( GridCell& gcl, Cohort& actingCohort, MadingleyInitialisation& params ) {
    ...
    for( int FunctionalGroup: mFunctionalGroupIndicesToEat ) {
        ...
        for( int i = 0; i < gcl.mStocks[FunctionalGroup].size( ); i++ ) {
            ...
            mEdibleMass = gcl.mStocks[FunctionalGroup][i].mTotalBiomass * 0.1;

            // Calculate the potential biomass eaten from this stock by the acting cohort
            mPotentialBiomassesEaten[FunctionalGroup][i] = CalculatePotentialBiomassEatenTerrestrial( mEdibleMass, mBodyMassHerbivore );
...
void EatingHerbivory::Run( GridCell& gcl, Cohort& actingCohort, unsigned currentTimestep, MadingleyInitialisation& params ) {
    ...
    for( int FunctionalGroup: mFunctionalGroupIndicesToEat ) {
        ...
        for( int i = 0; i < gcl.mStocks[FunctionalGroup].size( ); i++ ) {
            ...
            if( actingCohort.mCohortAbundance > 0 )Cohort::mIndividualMassAccounting["biomass"]["herbivory"] += mBiomassesEaten[FunctionalGroup][i] * mAssimilationEfficiency / actingCohort.mCohortAbundance;

            // Move the biomass eaten but not assimilated by an individual into the organic matter pool
            Cohort::mIndividualMassAccounting["organicpool"]["herbivory"] += mBiomassesEaten[FunctionalGroup][i] * ( 1 - mAssimilationEfficiency );
        }

        // Check that the delta biomass from eating for the acting cohort is not negative
        assert( Cohort::mIndividualMassAccounting["biomass"]["herbivory"] >= 0 && "Delta biomass from herbviory is negative" );

        // Calculate the total biomass eaten by the acting (herbivore) cohort
        mTotalBiomassEatenByCohort = Cohort::mIndividualMassAccounting[ "biomass" ][ "herbivory" ] * actingCohort.mCohortAbundance;
    }
}

double EatingHerbivory::CalculatePotentialBiomassEatenTerrestrial( double autotrophBiomass, double herbivoreIndividualMass ) {
    ...
    double AutotrophBiomassDensity = autotrophBiomass / mCellAreaHectares;
