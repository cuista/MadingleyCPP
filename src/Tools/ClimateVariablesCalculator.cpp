...
tuple<vector<double>, double, double> ClimateVariablesCalculator::MonthlyActualEvapotranspirationSoilMoisture( double AvailableWaterCapacity, vector<double> Precipitation, vector<double> MonthlyTemperatures ) {
    // Vector to hold potential evapotranspiration data
    vector<double> PotentialEvapotranspiration( 12 );
    ...
    vector<double> ActualEvapotranspiration( 12 ); // Will store monthly actual avapotranspiration (mm)
    vector<double> SoilWater( 12 ); // Will store montly soil water
    vector<double> DailyAET( 30 ); // Temporary store for daily actual evapotranspiration
    vector<double> DailySWC( 30 ); // Temporary store for daily soil water content
    vector<double> MidMonthDailyPET( 12 ); // The daily PET at the middle of each month
    vector<double> MidMonthDailyPPT( 12 ); // The daily PPT at the middle of each month
    ...
    return tuple<vector<double>, double, double>( ActualEvapotranspiration, SoilWaterPast, LengthOfFireSeason ); // return the collection of results
}
...
double ClimateVariablesCalculator::ApproximateNDF( vector<double> MATData ) {
    int PrevMonth;
    int NextMonth;
    double NDF = 0;
